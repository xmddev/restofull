...
import { INITIAL_MENU, INITIAL_TABLES, INITIAL_ORDERS } from '../constants';
...
const OrderEntry: React.FC = () => {
  ...
  // View State: 'SUMMARY' (ReadOnly Comanda) or 'MENU' (Add Items)
  // Initialize based on table status strictly
  ...
  // --- VIEW 1: SUMMARY (Occupied Table Initial View) ---
  if (viewMode === 'SUMMARY') {
    return (
  ...
  // --- VIEW 2: MENU SELECTION (Same as before but with Back button to Summary) ---
  
  // Render the Cart Content (Used in both Desktop Sidebar and Mobile Drawer)
  const renderCartContent = () => (
    <div className="flex flex-col h-full">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-6">
            ...
            {/* SECTION 1: EXISTING ORDER (READ ONLY) */}
            {existingItems.length > 0 && (
      ...
      <div className="flex-1 flex flex-col min-w-0 pb-20 lg:pb-0">
        ...
        <div className="bg-white p-4 shadow-sm z-10 flex flex-col md:flex-row md:items-center gap-4">
          <div className="flex items-center gap-4">
            {/* If occupied, back button goes to SUMMARY view, else to TABLES */}
            <button 
        ...
        <div className="flex-1 overflow-y-auto p-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 content-start pb-24">
          {filteredProducts.map(product => {
            // Check Inventory Logic (Simple Simulation)
